...
- Prospector Meteorite scanner と組み合わせる場合、追加で `CONFIG_BT_EXT_ADV=y` + `CONFIG_PROSPECTOR_STATUS_ADV_V2_EXT=y` を設定し、v2 Extended Advertising を有効化します。v2 ADV はレイヤー名リストと Meteorite custom config (OS / CPI / scroll layer) を 1Hz で配信し、v1 legacy ADV と並走します。
